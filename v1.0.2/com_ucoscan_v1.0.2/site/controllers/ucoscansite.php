...
class UcoscanControllerUcoscansite extends JControllerForm
{
    ...
    $msg_check_domain_selection = 'Please select a Domain - There must be at least one published Domain';
        $msg_check_suffix_selection = 'Please select a Suffix - There must be at least one published Suffix';
        ...
        if ($data[id] == '0') {
            ...
            $check_suffix_selection = $model->check_suffix_selection($data);            

            if (($check_No_Of_Chars) && ($check_cms_selection) && ($legth_of_domain<4) 
                    && ($check_domain_selection)
                    && ($check_suffix_selection)) {
                $sites_ok = $model->constructUrls($data);
                if ($sites_ok) {
                    parent::save($data, $urlVar);
                }
            } else
            if ((!$check_No_Of_Chars) || (!$check_cms_selection) || ($legth_of_domain>3)
                    || (!$check_domain_selection) || (!$check_suffix_selection)) {
                ...
                if (!$check_domain_selection) {
                 ...
                 if (!$check_suffix_selection) {
                    $this->setRedirect($url, $msg_check_suffix_selection);
                    return false;
                }                
            }
        } else {
